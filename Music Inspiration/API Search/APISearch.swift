...
var songFound = MusicAlbum(id: "", artistName: "", albumName: "", songName: "", genre: "", rating: "", releaseDate: "", coverPhotoFilename: "", musicVideID: "")
...
public func getApiDataSongAndArtist(songN: String, artistN: String) {
    
    let songP = songN.replacingOccurrences(of: " ", with: "+")
    let artistP = artistN.replacingOccurrences(of: " ", with: "+")
    
    /*
     Search query q=a will get all of the 460 national parks data with limit=500.
     If the park name = parkName given as input parameter, then we obtain its data.
     */
    let apiSearchQuery = "https://theaudiodb.com/api/v1/json/\(myApiKey)/searchtrack.php?s=\(artistP)&t=\(songP)"
        ...
        */
        ...
        do {
            ...
            */
            ...
            let s = dataJsonArray[0] as! [String: Any]
            songFound.id = UUID().uuidString
            if let name = s["strArtist"] as? String {
                songFound.artistName = name
            }
            if let name = s["strAlbum"] as? String {
                songFound.albumName = name
            }
            if let name = s["strTrack"] as? String {
                songFound.songName = name
            }
            if let name = s["strTrackThumb"] as? String {
                let n = name.replacingOccurrences(of: "\\", with: "")
                songFound.coverPhotoFilename = n
            }
            if let name = s["strGenre"] as? String {
                songFound.genre = name
            }
            
            if let name = s["strMusicVid"] as? String {
                songFound.musicVideID = name
            }
